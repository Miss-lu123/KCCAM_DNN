...
class AttentionLayer(tf.keras.layers.Layer):
    def __init__(self, input_dim, hidden_dim, attention_dim):
        ...
        self.W = tf.keras.layers.Dense(units=hidden_dim, activation='relu')
        self.U = tf.keras.layers.Dense(units=attention_dim, activation='relu')
        self.alpha = tf.keras.layers.Dense(units=1)

    def call(self, inputs):
        # inputs: [batch_size, input_dim]
        hidden = self.W(inputs)  # [batch_size, hidden_dim]
        attn_weights = tf.nn.softmax(self.alpha(tf.nn.relu(self.U(hidden))), axis=1)
        # [batch_size, 1]
        weighted_input = inputs * attn_weights  # [batch_size, input_dim]
...
def create_model(data):
    model = keras.Sequential([
        ...
        layers.Dense(16, activation='relu'),
        AttentionLayer(input_dim=16, hidden_dim=50, attention_dim=10),
